...
############main script#################
mu = float(sys.argv[1])         #mutation rate
...
scoef = 0.1                     #selection coefficient
adv_prob = pow(10,-5)           #rate of advantageous mutations

if met_time > 1:
    met_time = int(met_time)

ss_file = open("SimParamStats_SS_u"+str(mu)+"_metTime"+str(met_time)+"_"+str(repl)+".txt","w")
...
for itree in range(0,len(pri_tree_array)):
    ...
    neu_mlist,sel_mlist,mutlineage,new_mut_index = initiateFirstDeme_selection(starting_cells,mutlineage,new_mut_index,m_prob,scoef)
