...
def initiateFirstDeme_neutral(initial_cells,lineage,cur_index,division_prob):
    gsize = len(initial_cells)
    gmlist = list(initial_cells)
    while gsize < 5000: #10000 is the number of cells in a tumor deme
        divcells =  int(gsize*division_prob+1) #number of dividing cells in this generation
        gmlist = random.sample(gmlist,divcells)*2 #the mutation distribution in the sampled cells.
        gsize = divcells*2
        new_mut = np.random.poisson(mu*gsize)
        mut_assignment = Counter(np.random.choice(gsize,new_mut))
        for x in mut_assignment.keys():
            ...
            for xi in range(0,mmut):
                ...
                lineage += [gmlist[x]]
            gmlist[x] = mut_str
    return gmlist,lineage,cur_index
...
def initiateFirstDeme_selection(initial_cells,lineage,cur_index,division_prob,sfit):
    """
    ...
    cur_index - the starting mutation ID
...
def MutationCounter(mlist,mlineage,size_par,mean_depth,cutoff):
    """
...
def MutationCounterCCF(mlist,mlineage,size_par,mean_depth,cutoff1,cutoff2,cutoff3,cutoff4):
    """
    Find the pervasive mutations with high frequency in the sample (e.g. >50% of cells)
    """
    all_curindex = list(mlist)
    ...
    for y in index_dist.keys():
        ...
        all_mutindex += ylineage*index_dist[y]
    mut_dist = Counter(all_mutindex)
    testCCF1,testCCF2,testCCF3,testCCF4 = {},{},{},{}
    for x in mut_dist.keys():
        site_ccf = mut_dist[x]*1.0/test_size
        if site_ccf >= cutoff1:
            testCCF1[x] = site_ccf
        if site_ccf >= cutoff2:
            testCCF2[x] = site_ccf
        ...
        if site_ccf >= cutoff4:
            testCCF4[x] = site_ccf
    return testCCF1,testCCF2,testCCF3,testCCF4

def pubmutGenerator(n,size_par,mean_depth):
    prob_par=size_par*1.0/(size_par+mean_depth)
    mean_alleleFreq = 0.5
    pub_alleleFreq = []
    for i in range(0,n):
        site_depth = np.random.negative_binomial(size_par,prob_par)
        if site_depth >= 10:
            var_reads = np.random.binomial(site_depth,mean_alleleFreq)
            site_maf = var_reads*1.0/site_depth
            pub_alleleFreq += [(site_depth,site_maf)]
        else:
            pub_alleleFreq += [(site_depth,0.51)]
    return pub_alleleFreq

def keys2mutIndex(sp,sample_keys):
    kindex = []
    for key in sample_keys:
        kindex += sp[key].mutlist
    return kindex

def lineageCompare(index1,index2,mlineage):
    total_compare = len(index1)*len(index2)
    divergence = np.array([0,0,0])
    for d1 in index1:
        for d2 in index2:
            lineage1 = sorted(traceLineage(mlineage,d1))
            lineage2 = sorted(traceLineage(mlineage,d2))
            if len(lineage1) == 0:
                Lcom = 0
                Lp = 0
                Lm = len(lineage2)
                ...
                Lp = len(lineage1)
                Lm = 0
            elif len(lineage1) < len(lineage2):
                short = len(lineage1)
                share = 0
		for si in range(0,short):
                    if lineage1[si] == lineage2[si]:
			share += 1
                Lcom = share
                Lp = len(lineage1)-share
                Lm = len(lineage2)-share
            else:
                short = len(lineage2)
                share = 0
		for si in range(0,short):
			...
			share += 1
                ...
                Lp = len(lineage1)-share
                Lm = len(lineage2)-share
            divergence += np.array([Lcom,Lp,Lm])
    diverdata = list(divergence)
    diverdata = [x*1.0/total_compare for x in diverdata]
    return diverdata
...
def filter_mutation(allelefreq,cutoff):
    sub_mut = []
    for x in allelefreq.keys():
        if allelefreq[x][1] <= cutoff:
            sub_mut += [x]
    return sub_mut

def collect_periphery(sp,current):
    peri = []
    for key in current:
...
def centre_muts(pridict,metdict,cutoff1,cutoff2):
    share_muts = sets.Set(pridict.keys()) & sets.Set(metdict.keys())
    ncentre = 0
    for ckey in share_muts:
        if pridict[ckey]<cutoff1 and metdict[ckey]<cutoff2:
            ncentre += 1
    return ncentre

############main script#################
mu = float(sys.argv[1])
met_time = float(sys.argv[2])
ptree = str(sys.argv[3])
mtree = str(sys.argv[4])
repl = int(sys.argv[5])
...
inferI_file = open("SimulParamStats_SS_u"+str(mu)+"_metTime"+str(met_time)+"_"+str(repl)+".txt","w")
inferI_file.write("mu"+" "+"cluster.size"+" "+"met.time"+" "+"S1"+" "+"S2"+" "+"S3"+" "+"S4"+" "+"S5"+" "+"S6"+" "+"S7"+" "+"S8"+" "+"S9")
inferI_file.write("\n")
...
for itree in range(0,len(pri_tree_array)):
    ...
    while first_deme_size < 5000:
        n1,n2 = len(flist),len(sel_flist)                         #n1 and n2 are the current number of neutral founder cells and advantageous cells, respectively
        divcells =  int(n1*p_prob+1)                        #number of dividing cells of neutral lineage in this generation. The other cells will die in the next generation
        flist = random.sample(flist,divcells)*2
        if n2 > 0:
            sel_divcells = lowerORupper(n2*p_prob*(1+scoef))   #number of dividing cells of advantageous lineage in this generation        
            sel_flist = random.sample(sel_flist,sel_divcells)*2
        n1,n2 = len(flist),len(sel_flist)
        first_deme_size = n1+n2
        if n1 > 0:
            new_mut1 = np.random.poisson(mu*n1)               # the total number of mutations occurring in a generation follows Poission distribution with lambda=u*n
            mut_assig1 = Counter(np.random.choice(n1,new_mut1))
            for x1 in mut_assig1.keys():
                nmut = mut_assig1[x1]
                new_mut1 = range(mindex+1,mindex+1+nmut)
                mut_str = ",".join(map(str,new_mut1))
                #if nmut > 1:
                #    for t in new_mut1:
                #        multi_events[str(t)] = mut_str
        ...
        if n2 > 0:
            ...
            for x2 in mut_assig2.keys():
                nmut = mut_assig2[x2]
                new_mut2 = range(mindex+1,mindex+1+nmut)
                mut_str = ",".join(map(str,new_mut2))
                #if nmut > 1:
                #    for t in new_mut2:
                #        multi_events[str(t)] = mut_str
                for xn in range(0,nmut):
                    mindex += 1
                    mutlineage += [sel_flist[x2]]
                sel_flist[x2] = mut_str
        
        if random.random() < adv_prob*n1:                           # occurence of advantageous mutation on the neutral lineage
            mindex += 1
            current_n1 = len(flist)
            mutlineage += [str(flist[current_n1-1])]
            sel_flist += [str(mindex)]
            flist = flist[0:current_n1-1]
    
        if met_time < 2 and done==0 and first_deme_size > 1000:
            anc_list = list(flist+sel_flist)
            fmet = random.sample(anc_list,cluster_size)
            done = 1
    
    print len(flist),len(sel_flist)
    
    pmut_index = int(mindex)
    #print "starting_mut_index=",pmut_index,len(mutlineage)-1
    
    proot = build_tree(cur_ptree)
    core1,core2,core3,core4,met_deme = [],[],[],[],[]

    mut_simulatorPri_selection(proot,flist,sel_flist,mutlineage,pmut_index,p_prob,scoef,core1,core2,core3,core4,met_deme)
    #mut_simulatorPri_neutral(proot,flist,mutlineage,pmut_index,p_prob,core1,core2,core3,core4,met_deme)
    pmut_index = len(mutlineage)-1
    #print "final_mut_index=",pmut_index

    pri_merge = core1+core2+core3+core4
    #pri_merge = core1
    maf_primary = MutationCounter(pri_merge,mutlineage,2,100,0.05)
    
    ####growth of met####
    if met_time >= 2:
        fmet = random.sample(met_deme,cluster_size)
    
    #for clusize in met_cluster.keys():
    starting_cells = list(fmet)
    new_mut_index = int(pmut_index)
    #new_lineage = list(mutlineage)
    
    starting_deme,sel_mlist,mutlineage,new_mut_index = initiateFirstDeme_selection(starting_cells,mutlineage,new_mut_index,m_prob,scoef)
    mroot = build_tree(cur_mtree)
    tissue1,tissue2,tissue3,tissue4 = [],[],[],[]
    #mut_simulatorMet_neutral(mroot,starting_deme,mutlineage,new_mut_index,m_prob,tissue1,tissue2,tissue3,tissue4)
    mut_simulatorMet_selection(mroot,starting_deme,sel_mlist,mutlineage,new_mut_index,m_prob,scoef,tissue1,tissue2,tissue3,tissue4)
   
    met_merge = tissue1+tissue2+tissue3+tissue4
    #met_merge = tissue1
    maf_met = MutationCounter(met_merge,mutlineage,2,100,0.05)
    ...
    inferI_file.write(str(mu)+" "+str(cluster_size)+" "+str(met_time)+" "+str(S1)+" "+str(S2)+" "+str(S3)+" "+str(S4)+" "+str(S5)+" "+str(S6)+" "+str(S7)+" "+str(S8)+" "+str(S9))
    inferI_file.write("\n")
