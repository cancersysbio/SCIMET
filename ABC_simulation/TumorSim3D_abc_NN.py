...
inferI_file = open("SimulParamStats_NN_u"+str(mu)+"_metTime"+str(met_time)+"_"+str(repl)+".txt","w")
...
for itree in range(0,len(pri_tree_array)):
    ...
    flag = 0
    while deme_size < 5000: #5000 is the number of cells in a tumor deme
        Ndivcells =  int(deme_size*p_prob+1) #number of dividing cells in this generation
        ...
        for x in mut_assignment.keys():
            ...
            for xi in range(0,mmut):
                ...
                mutlineage += [flist[x]]
            flist[x] = mut_str
        if met_time < 2 and flag==0 and deme_size > 1000:
