...
def start_a_deme(initial_cells,lineage,cur_index,division_prob):
...
inferI_file = open("SimulParamStats_NS_u"+str(mu)+"_metTime"+str(met_time)+"_"+str(repl)+".txt","w")
...
for itree in range(0,len(pri_tree_array)):
    ...
    while first_deme_size < 5000: #5000 is the number of cells in a tumor deme
        Ndivcells =  int(first_deme_size*p_prob+1) #number of dividing cells in this generation
        flist = random.sample(flist,Ndivcells)*2 #the mutation distribution in the sampled cells.
        first_deme_size = Ndivcells*2
        new_mutation = np.random.poisson(mu*first_deme_size)
        mut_assignment = Counter(np.random.choice(first_deme_size,new_mutation))
        for x in mut_assignment.keys():
            mmut = mut_assignment[x]
